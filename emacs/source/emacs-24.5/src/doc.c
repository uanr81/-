/* Записывать индексы строк функций, хранящихся в файле.
...
#include <errno.h>
#include <sys/types.h>
#include <sys/file.h>	/* Должно быть после sys/types.h для USG.  */
#include <fcntl.h>
#include <unistd.h>
...
/* Буфер, используемый для чтения из файла документации.  */
static char *get_doc_string_buffer;
static ptrdiff_t get_doc_string_buffer_size;

static unsigned char *read_bytecode_pointer;

/* `readchar' в lread.c переадресовывает сюда, чтобы получить следующий байт.
   Если UNREADFLAG равно 1, мы читаем непрочитанный байт.  */
...
/* Извлечение строки документа из файла.  FILEPOS говорит, где его взять.
   если это целое число, используйте это положение в стандарте DOC файла.
   Если это (FILE . INTEGER), используйте FILE как имя файла
   а также INTEGER как позицию в этом файле.
   Но если INTEGER отрицательно, сделайте его положительным.
   (Отрицательное целое число используется для пользовательских переменных, поэтому
   мы можем различать их без фактической выборки строки документа.)

   Если местоположение не указывает на начало строки документа
   (например, потому что файл был изменен и местоположение устарело)
   return nil.

   Если UNIBYTE, всегда создавайте unibyte-строку.

   Если DEFINITION, Предположим, что это для чтения
   определение динамической функции; Преобразовать байтовую строку
   ивектор констант с соответствующей обработкой байтов,
   и вернуть cons-ячейку.  */

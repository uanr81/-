...
void fillDeck( Card * const wDeck, const char * wFace[], 
	       const char * wSuit[] );//создание колоды от туза до короля
void shuffle( Card * const wDeck );//тасование карт
void deal( const Card * const wDeck );//раздача перетасованных карт
...
{ 
   ...
   srand( time( NULL ) ); /* Рандомизировать(генерировать различные последовательности
случайных чисел, аргумент здесь семя для rand ) */

   fillDeck( deck, face, suit ); /* Создаёт колоду Cards */
   shuffle( deck ); /* перетасовать карты */
   deal( deck ); /* Раздать перетасованные карты */
